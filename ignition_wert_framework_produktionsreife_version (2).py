...
# ==============================================================================
import subprocess
subprocess.check_call(['pip', 'install', 'flask', 'flask-cors', 'xgboost', 'torch', 'pandas', 'numpy', 'scikit-learn', 'google-cloud-storage', 'pyngrok', 'joblib', 'optuna', 'flask-socketio', 'eventlet', 'backtrader', 'celery', 'redis', 'pandas_ta'])
...
def _create_features(df):
    ...
    VERBESSERT: Fügt RSI, MACD, ATR und ADX hinzu. Korrigiert Filling-Order für Time-Series.
    """
    df_copy = df.copy()
    ...
    df_copy.fillna(method='ffill', inplace=True)  # Zuerst forward-fill (vergangene Werte)
    df_copy.fillna(method='bfill', inplace=True)  # Dann backward-fill (nur für initiale NAs)
    return df_copy

def get_feature_columns(df):
    """Dynamische Feature-Spalten basierend auf verfügbaren Indikatoren."""
    base = ['open', 'high', 'low', 'close', 'volume', 'range', 'avg_price']
...
def prepare_sequences(df, window_size, prediction_length, step=1, scaler=None):
    ...
    if scaler is None:
        scaler = MinMaxScaler().fit(df_with_features[feature_cols])
...
def train_lstm_async(df, hparams, ticker):
    ...
    # Modell-Registrierung aktualisieren
    model_registry = {}
    ...
    model_registry[ticker] = {
        "lstm": best_model_path,
        "lstm_scaler": best_scaler_path,
...
# ==============================================================================
# 9. Backtesting-Funktion mit Walk-Forward-Validation
# ==============================================================================
def walk_forward_validation(df, hparams, n_splits=5):
    """Walk-Forward-Validation für robustes Backtesting ohne Look-Ahead-Bias."""
    results = []
    test_size = len(df) // n_splits
    
    for i in range(1, n_splits + 1):
        train_end = len(df) - test_size * i
        ...
        # Neu trainieren auf historischen Daten
        lstm_model_path, lstm_scaler_path, _ = train_lstm_async(train_df, hparams, 'wf_temp').get()  # Sync für Backtest
        trained_lstm = Seq2Seq(Encoder(Config.LSTM_HPARAMS['input_dim'], Config.LSTM_HPARAMS['hidden_dim'], Config.LSTM_HPARAMS['n_layers'], Config.LSTM_HPARAMS['dropout_prob']),
                               Decoder(Config.LSTM_HPARAMS['input_dim'], Config.LSTM_HPARAMS['hidden_dim'], Config.LSTM_HPARAMS['n_layers'], Config.LSTM_HPARAMS['dropout_prob']), device=torch.device('cpu'))
        trained_lstm.load_state_dict(torch.load(lstm_model_path))
        lstm_scaler = joblib.load(lstm_scaler_path)

        xgboost_model_path, _ = train_xgboost_async(train_df, Config.XGBOOST_HPARAMS, 'wf_temp').get()
        trained_xgboost = joblib.load(xgboost_model_path)

        # Backtest auf Testperiode
        cerebro = bt.Cerebro()
        cerebro.addstrategy(BreakoutStrategy, precomputed_df=_create_features(test_df), trained_lstm=trained_lstm, lstm_scaler=lstm_scaler, trained_xgboost=trained_xgboost)
        data = bt.feeds.PandasData(dataname=test_df.set_index('date'))
        ...
        period_return = (final_portfolio - Config.BACKTEST_INITIAL_CASH) / Config.BACKTEST_INITIAL_CASH * 100
        results.append(period_return)

    avg_return = np.mean(results)
    sharpe = np.mean(results) / np.std(results) * np.sqrt(252 / n_splits) if len(results) > 0 else 0
    return {'avg_returns_percent': avg_return, 'sharpe_ratio': sharpe}
...
class BreakoutStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        features = current_df[get_feature_columns(current_df)].values[-1:]  # Use precomputed
...
def load_latest_model(ticker):
    if not os.path.exists(Config.MODEL_REGISTRY_PATH):
    ...
    if ticker not in registry:
        raise ValueError(f"No model for ticker {ticker}")

    latest = registry[ticker]
    ...
    model = Seq2Seq(encoder, decoder, device).to(device)
    model.load_state_dict(torch.load(latest["lstm"]))
    model.eval()
    scaler = joblib.load(latest["lstm_scaler"])
    xgboost = joblib.load(latest["xgboost"])
...
def start_training(ticker):
    ...
    if file and file.filename.endswith('.csv'):
        ...
        # Starte async Tuning und Training
        tune_lstm_hyperparameters_async.delay(df, ticker)
        tune_xgboost_hyperparameters_async.delay(df, ticker)
        lstm_task = train_lstm_async.delay(df, Config.LSTM_HPARAMS, ticker)
        xgboost_task = train_xgboost_async.delay(df, Config.XGBOOST_HPARAMS, ticker)

        return jsonify({'lstm_task_id': lstm_task.id, 'xgboost_task_id': xgboost_task.id})
...
def check_status(task_id):
    ...
    if task.state == 'PENDING':
        ...
        response = {'state': task.state, 'status': str(task.info)}  # Exception
    return jsonify(response)

@app.route('/predict/<ticker>', methods=['GET'])
def get_prediction(ticker):
    try:
        trained_lstm, lstm_scaler, trained_xgboost = load_latest_model(ticker)
    except FileNotFoundError:
        return jsonify({'error': 'Modell für diesen Ticker nicht gefunden. Training abschließen.'}), 404

    # Annahme: Daten aus GCS laden oder Cache
    # Für Demo: Dummy-Daten, in Prod: GCS laden
    df = pd.read_csv('test.csv')  # Ersetze durch GCS-Laden
    ...
    if sequences.size == 0:
        return jsonify({'error': 'Nicht genügend Daten für eine Vorhersage.'}), 400
...
def run_backtest():
    ...
    if file.filename.endswith('.csv'):
        ...
        results = walk_forward_validation(df, Config.LSTM_HPARAMS)

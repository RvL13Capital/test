...
# ==============================================================================
!pip install flask flask-cors xgboost torch pandas numpy scikit-learn google-cloud-storage pyngrok joblib optuna flask-socketio eventlet backtrader
...
def train_lstm(df, hparams):
    ...
    loader = DataLoader(dataset, batch_size=16, shuffle=True)  # Optimiert: Kleinere Batch-Size für schnellere Iterationen
...
# ==============================================================================
# 8. Hyperparameter-Tuning mit Optuna
# ==============================================================================
def tune_lstm_hyperparameters(df):
    """Findet die besten Hyperparameter für LSTM mit Optuna."""
    train_df, val_df = train_test_split(df, test_size=0.2, shuffle=False)
    src, trg, scaler = prepare_sequences(train_df, Config.DATA_WINDOW_SIZE, Config.DATA_PREDICTION_LENGTH)
    val_src, val_trg, _ = prepare_sequences(val_df, Config.DATA_WINDOW_SIZE, Config.DATA_PREDICTION_LENGTH)
    ...
    def objective(trial):
        params = {
            ...
            'epochs': 20  # Reduziert für Tuning
        }
        ...
        trg_tensor = torch.tensor(trg, dtype=torch.float32, device=device)

        val_src_tensor = torch.tensor(val_src, dtype=torch.float32, device=device).to(device)
        val_trg_tensor = torch.tensor(val_trg, dtype=torch.float32, device=device).to(device)

        for epoch in range(params['epochs']):
            model.train()
            output = model(src_tensor, trg_tensor)
            loss = criterion(output[:, 1:, :], trg_tensor[:,1:,1: :])
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    print("Beste LSTM-Parameter gefunden: ", study.best_params)
...
def tune_xgboost_hyperparameters(df):
    ...
    study = optuna.create_study(direction='minimize')
    study.optimize(objective, n_trials=10)  # Optimiert: Weniger Trials für schnellere Ausführung
...
class BreakoutStrategy(bt.Strategy):
    def __init__(self, df, trained_lstm, lstm_scaler, trained_xgboost):
        self.df = df
    ...
    def next(self):
        if self.bar_num >= len(self.df) - Config.DATA_WINDOW_SIZE:
            return

        current_df = self.df.iloc[:self.bar_num + Config.DATA_WINDOW_SIZE]
        ...
        features = _create_features(current_df.iloc[-1:])[feature_cols].values
...
def backtest_models(df, trained_lstm, lstm_scaler, trained_xgboost):
    ...
    cerebro = bt.Cerebro()
    cerebro.addstrategy(BreakoutStrategy, df=df, trained_lstm=trained_lstm, lstm_scaler=lstm_scaler, trained_xgboost=trained_xgboost)
    ...
    # Einfache Sharpe-Ratio (annuallisiert, risikofreier Rate 0)
...
# ==============================================================================
# 10. Hauptausführung: Tuning, Training, Backtesting und Laden der Modelle
# ==============================================================================
# Erzeuge Dummy-CSV mit mehr Daten
df = pd.DataFrame({
    'date': pd.date_range(start='2022-01-01', periods=1000),  # Optimiert: Mehr Daten für Backtesting
    'open': np.linspace(100, 150, 1000) + np.random.randn(1000) * 2,
    'high': np.linspace(102, 155, 1000) + np.random.randn(1000) * 2,
    'low': np.linspace(98, 148, 1000) + np.random.randn(1000) * 2,
    'close': np.linspace(101, 152, 1000) + np.random.randn(1000) * 2,
    'volume': np.linspace(10000, 50000, 1000) + np.random.randn(1000) * 1000
})
df.to_csv('test.csv', index=False)

# --- Hyperparameter-Tuning ---
best_lstm_params = tune_lstm_hyperparameters(df)
Config.LSTM_HPARAMS.update(best_lstm_params)
...
xgboost_model_path, _ = train_xgboost(df, Config.XGBOOST_HPARAMS)
...
# --- Backtesting ---
backtest_results = backtest_models(df, trained_lstm, lstm_scaler, trained_xgboost)
...
def run_backtest():
    ...
    if file.filename.endswith('.csv'):
        df = pd.read_csv(file.stream)
        df['date'] = pd.to_datetime(df['date'])  # Sicherstellen, dass 'date' vorhanden ist

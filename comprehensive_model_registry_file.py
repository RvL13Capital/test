# project/comprehensive_model_registry.py
"""
...
import pandas as pd
import numpy as np
from sqlalchemy import create_engine, Column, String, Float, DateTime, JSON, Integer, Boolean, func
...
class ModelRegistry:
    ...
    def _generate_recommendation(self, comparison: Dict) -> str:
        ...
        improvements = 0
        if delta.get('mse_change_pct', 0) < -5:
            improvements += 1
        if delta.get('sharpe_ratio_change_pct', 0) > 5:
            improvements += 1
        if delta.get('directional_accuracy_change_pct', 0) > 2:
            improvements += 1
        
        if improvements >= 2:
            return "PROMOTE: Candidate shows significant improvement"
        elif improvements == 1:
            return "REVIEW: Candidate shows minor improvement"
        else:
            return "RETAIN: Baseline performs better"
...
class ModelRegistryCLI:
    ...
    def list_models(self, model_name: str = None, stage: str = None):
        ...
        session = self.registry.SessionLocal()
        try:
